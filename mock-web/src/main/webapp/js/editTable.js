...
$(function() {
	$.fn.handleTable = function(options) {
		...
		function createLink(str) {
			return "<a href=\"javascript:void(0)\" style=\"margin:0 3px\">"
					+ str + "</a>";
		}
		/**
		 * 初始各种操作的链接
		...
		function createEdit(operateCol) {
							...
							function() {
								if (replaceQuote($(this).html()) == replaceQuote(c.edit)) { // 如果此时是编辑状态
									...
									for ( var i = 0; i < cols.length; i++) {
										...
										var inputValue = tr.children('input')
												.val();
			...
			function toSave(ele) {
				...
				for ( var i = 0; i < cols.length; i++) {
					var tr = p.children().eq(cols[i]);
					var editTr = "<input type=\"text\" class=\"form-control\" value=\""
							+ tr.html() + "\"/>";
